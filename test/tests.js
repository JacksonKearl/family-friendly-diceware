...
describe('diceware', function() {
  ...
  describe('clean_phrase', function() {
    it('cleans a clean phrase', function() {
      var val = diceware.clean_phrase('turtle astride porous');
      return val.then(function(phrase) {
        phrase.should.be.an.instanceOf(String).and.equal('turtle astride porous')
      })
    })

    it('accepts a noisy phrase and returns the clean version', function() {
      var val = diceware.clean_phrase('   tUrtLe      astride porouS  ');
      return val.then(function(phrase) {
        phrase.should.be.an.instanceOf(String).and.equal('turtle astride porous')
      })
    ...
    it('is able to use the fixer function to correct mistakes in a phrase', function() {
      var val = diceware.clean_phrase('turTal astride porous', function(str) {
        if (str === "turtal")
          return Promise.resolve('turtle');
        }
      );
      return val.then(function(phrase) {
        phrase.should.be.an.instanceOf(String).and.equal('turtle astride porous')
