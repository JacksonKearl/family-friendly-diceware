...
var clean_phrase = function(phrase, async_fixer) {
  var phrase_arr = phrase.trim().toLowerCase().split(' ');

  var promises = phrase_arr.map((word, i) => {
    if (word.length) {
      if (words.str_to_num[word]) {
        return Promise.resolve(word)
      } else {
        return async_fixer(word)
      }
    } else {
  ...
  return Promise.all(promises).then((results) => results.filter(val => val != '').join(' '));
}
